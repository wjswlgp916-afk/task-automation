...
| 검수확인서 (`inspection`) | HWP + PDF | **검수항목 표를 선택한 도구·부가서비스에 맞춰 자동으로 골라 남김** (표에 없는 항목은 행 자체를 삭제, 라벨 칸의 세로병합 수도 자동 조정). 베이직 등급은 검수확인서 대상이 아니라서, 프리미어가 하나도 없으면 오류. 작업 시작일·종료일을 추가로 입력받음(둘 다 필수, 기본값 없음). 용역명 규칙은 위와 동일 |
...
src/quote_automation/
  ...
  inspection_writer.py 검수확인서 전용 렌더러 (등급별 표 행 자동 선택, HWP+PDF)
...
1. **다른 서류 추가** — 과업지시서·독점공급확인서·보안확약서·수의계약확인서 등.
   (검수확인서에서 이미 "등급에 따라 표 행을 자동으로 골라 남기는" 패턴을
   구현했으므로, 비슷한 조건부 로직이 필요한 서류도 참고해 만들 수 있다.)
