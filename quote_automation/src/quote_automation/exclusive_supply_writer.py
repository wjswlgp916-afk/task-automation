...
처음에는 메모 앵커(CTRL_HEADER)를 절대 건드리지 않고 내용만 공백으로
비우는 방식을 썼으나, 이 경우 앵커가 "메모가 달린 활성 필드"로 계속
남아있어 한글이 항상 초록색 음영 + 여백 책갈피 탭을 표시해버려(메모
내용이 비어 있어도) 실사용에 문제가 있었다.

이후 사용자가 실제 한글에서 원본 양식의 메모를 직접 삭제(우클릭 → 메모
삭제)해 저장한 파일을 받아 원본과 바이트 단위로 비교해본 결과, 한글
자신도 메모를 지울 때 CTRL_HEADER 와 MEMO_LIST 를 통째로 제거하고
(앵커를 남기지 않음), 감싸여 있던 본문 텍스트는 그대로 유지하며, 문단의
LINE_SEG(줄 나눔 캐시)는 전혀 재계산하지 않고 그대로 둔다는 것을
확인했다 — 즉 hwp_writer.strip_memo_controls() 가 이미 하고 있는 일과
구조적으로 거의 동일했다. 이 비교로 "CTRL_HEADER 를 지우면 무조건
높음 보안에서 막힌다"는 이전 결론은 잘못된 것이었다고 보고, 다시
strip_memo_controls() (완전 제거)로 되돌렸다. 이렇게 하면 메모 자체가
완전히 사라져 화면 표시 문제도 해결된다.

본문 문단은 K/U 단독일 때 set_plain_text() 로 통째로 다시 쓴다 — 메모가
이미 완전히 제거되어 인라인 필드 마커가 하나도 남아있지 않으므로, 길이가
달라져도 안전하다(단, LINE_SEG 는 일부러 건드리지 않는다 — 억지로 1줄로
선언하면 한글이 모든 글자를 겹쳐 그리는 문제가 있었다, 아래 _rewrite_paragraph
참고).
...
def _rewrite_paragraph(records: List[Record], hdr_idx: int, txt_idx: int,
    ...
    건드리지 않는다 — 계약서 특이사항 문단과 한글 자신의 메모 삭제 결과에서
    똑같이 확인된 것처럼, 글자 수가 달라져도 그대로 둬야 안전하다(억지로
    1줄로 축소하면 한글이 모든 글자를 겹쳐 그린다: 실제로 겪은 문제)."""
